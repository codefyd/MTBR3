(() => {
  ...
  function render() {
    ...
    area.innerHTML = `
      ...
      <div class="modal-x hidden" id="secretModal"><div class="modal-card" style="max-width:700px">
        ...
        <label class="lbl">الرابط</label><div class="secret-box" id="mcpUrl"></div>
        <label class="lbl" style="margin-top:.8rem">Authorization Bearer</label><div class="secret-box" id="mcpSecret"></div>
        <button class="btn-x" id="copyMcpConfig" style="margin-top:1rem"><i class="fa-solid fa-copy"></i> نسخ إعداد JSON</button>
      </div></div>
  ...
  function wire() {
    ...
    document.querySelectorAll('.update-org-form').forEach(f => f.onsubmit = async e => {
      e.preventDefault(); try { await api('update_organization',{organization_id:f.organization_id.value,name:f.name.value,contact_email:f.contact_email.value,subscription_ends_at:f.subscription_ends_at.value,is_active:f.is_active.checked}); App.toast('تم تحديث الاشتراك'); await load(); } catch(err){App.toast(err.message,'err');}
    });
    document.querySelectorAll('.add-user-form').forEach(f => f.onsubmit = async e => {
      e.preventDefault(); try { await api('add_user',{organization_id:f.organization_id.value,email:f.email.value,temporary_password:f.temporary_password.value,role:f.role.value,allowed_pages:checked(f,'member_pages')}); App.toast('تمت إضافة المستخدم'); await load(); } catch(err){App.toast(err.message,'err');}
    });
    document.querySelectorAll('.create-token-form').forEach(f => f.onsubmit = async e => {
      e.preventDefault(); try { const out=await api('create_mcp_token',{organization_id:f.organization_id.value,user_id:f.user_id.value||null,name:f.name.value,expires_at:f.expires_at.value||null}); showSecret(out); await load(false); } catch(err){App.toast(err.message,'err');}
    });
    document.querySelectorAll('.revoke-token').forEach(b => b.onclick = async () => { if(!confirm('إلغاء هذا المفتاح فورًا؟'))return; try{await api('revoke_mcp_token',{token_id:b.dataset.id});App.toast('تم إلغاء المفتاح');await load();}catch(err){App.toast(err.message,'err');} });
    document.querySelectorAll('.edit-member').forEach(b => b.onclick = () => editMember(JSON.parse(b.dataset.member)));
    document.querySelectorAll('[data-close-secret]').forEach(b=>b.onclick=()=>document.getElementById('secretModal').classList.add('hidden'));
  ...
  function showSecret(out) {
    ...
    document.getElementById('copyMcpConfig').onclick=async()=>{
      const config={type:'streamable-http',url:out.mcp_url,headers:{Authorization:`Bearer ${out.token}`}};
      await navigator.clipboard.writeText(JSON.stringify(config,null,2)); App.toast('تم نسخ الإعداد');
    };
    modal.classList.remove('hidden');
  }
